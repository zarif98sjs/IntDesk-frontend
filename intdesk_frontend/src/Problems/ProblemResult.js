...
function ProblemResult() {
  ...
  const [remainingCases, setremainingCases] = useState(
    problem.input_outputs.length + 1
  );
  ...
  useEffect(() => {
    const addSolution = async () => {
      ...
      await axios
        .post(
          "http://localhost:8000/problems/problem/"
            .concat(problem.id)
            .concat("/solution/"),
          postData,
          {
      ...
      await axios
        ...
        .then((response) => {
          ...
          if (statusId === 1 || statusId === 2) {
            ...
            console.log(
              "input output was ",
              problem.input_outputs[i].input,
              problem.input_outputs[i].output
            );
