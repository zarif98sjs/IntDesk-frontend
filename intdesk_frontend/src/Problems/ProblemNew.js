...
function ProblemNew() {
  ...
  const submitRoles = async (id) => {
    ...
    let address = "http://localhost:8000/problems/problem/"
      .concat(id)
      .concat("/role/");
  ...
  const handleSubmit = async () => {
    ...
    if (mode === "POST") {
      ...
      console.log(mode);
      await axios
        .post("http://localhost:8000/problems/problem/", postData, {
          headers: {
            Authorization: "Token ".concat(authToken.token),
            "Content-Type": "application/json",
          },
      ...
      await axios
        .put(
          "http://localhost:8000/problems/problem/".concat(id).concat("/"),
          postData,
          {
            headers: {
              Authorization: "Token ".concat(authToken.token),
          ...
          .then((res) => {
            ...
            for (let i = 0; i < res.data.subcategories.length; i += 1) {
              prevSubcategories.push({
                category: res.data.subcategories[i].category,
                subcategory: res.data.subcategories[i].name,
              });
            }
